...
def get_choice(list):
    ...
    for vm in enumerate(list):
        if vm[1] != "":
            name = find_name(
                vm[1]
            )  # searches the string for " and returns the string between the quotes
            pprint(f"[yellow] {vm[0] + 1} [/yellow] {name}")
    try:
        return find_name(
            list[int(console.input("[red] Enter your choice: [/red] ")) - 1]
        )
    except:
...
def restart():
    name = get_choice(
        subprocess.check_output(
            "vboxmanage list runningvms", shell=True, universal_newlines=True
        ).split("\n")
    )
    try:
...
def shutdown():
    ...
    try:
        shutdown_message = subprocess.check_output(
            f"vboxmanage controlvm {name} poweroff soft",
            shell=True,
...
def startup(ssh: bool = True, headless: bool = True):
    ...
    state = subprocess.check_output(
        f"VBoxManage showvminfo {choice} | grep State",
        shell=True,
        universal_newlines=True,
    ).split("                       ")
    ip = (
    ...
    try:
        ...
        pprint(
            "[red] FAILURE! [/red] Something went wrong while turning on your machine, aborting..."
        )
        exit(-1)
    if ssh:
